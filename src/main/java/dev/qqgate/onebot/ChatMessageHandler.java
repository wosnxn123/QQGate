 ...
 *
 * 玩家指令：
 *   绑定 <码>       完成绑定（4~8 位数字，分隔符可选）
 *   解绑            列出自己的绑定（不执行）
 *   解绑 <账号名>    精确解绑自己名下该账号
 *   查询            列出自己的绑定
 *   帮助            指令列表
 *
 * 管理员指令（admins.qq 白名单 + admins.respond.group/private 通道开关）：
 *   查 <玩家名|QQ号>            双向查询绑定
 *   解绑 <玩家名|QQ号>          单条直解；多条列出
 *   解绑 <玩家名> <QQ号>        精确解绑一条
 *   全解绑 <玩家名|QQ号>        清空目标全部绑定
 *   绑定 <玩家名> <QQ号>        代绑（跳验证码，走限额裁决）
 *   状态                        连接状态与统计
 *   帮助                        管理员指令列表
 *
 * 分流规则：先按管理员语法匹配（查/全解绑/状态/三段式绑定），命中且身份+通道
 * 通过 → 管理路径；否则落回玩家路径。管理员发「绑定 <纯数字>」仍是玩家绑定自己。
 */
