...
### 玩家 QQ 指令（群内白名单群 / 私聊需开启）

| 指令 | 说明 |
|---|---|
| `绑定 4823` | 完成绑定（空格/全角空格/冒号可选，可紧贴） |
| `查询` | 查看自己的绑定列表 |
| `解绑` | 列出自己的绑定（不执行） |
| `解绑 <账号名>` | 精确解绑自己名下该账号（需 `bind.self-unbind: true`） |
| `帮助` | 指令列表 |

### 管理员游戏内命令 `/qqgateadmin`（权限 `qqgate.admin`，OP 默认）

| 子命令 | 说明 |
|---|---|
| `status` / `codes` / `diag` | 连接统计 / 待验证码 / 运行时配置自检（健康警告） |
| `lookup <玩家名\|QQ号>` | 双向查询（被拉黑 QQ 显示 ⚠ 标记） |
| `unbind <玩家名\|QQ号> [QQ号]` | 解绑（单条直解；多条列出；双参精确） |
| `unbindall <玩家名\|QQ号>` | 清空目标全部绑定 |
| `bind <玩家名> <QQ号>` | 代绑（跳过验证码，走限额裁决） |
...
| `解绑 <玩家名\|QQ号>` | 解绑（单条直解；多条列出引导精确） |
| `拉黑 <QQ号> [原因]` / `解拉黑 <QQ号>` / `拉黑列表` | QQ 黑名单（同游戏内 qqban 系列） |
| `状态` | 连接状态与统计 |
| `帮助` | 管理员指令列表 |

机器人的所有回复自动**引用触发消息**并 **@发送者**（私聊回执无引用/@）。
